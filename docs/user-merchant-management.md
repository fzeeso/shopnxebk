...
| `POST` | `/api/v1/platform/merchants` | `manage stores` | Atomically create a Store-scoped owner, draft Store, settings, platform domain, selected Theme license/published installed copy, active membership, and roles; returns `dashboard_url`. |
