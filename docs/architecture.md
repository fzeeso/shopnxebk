...
The application is a modular monolith: one Laravel process, one PostgreSQL shared schema, and explicit module boundaries. `Modules/Authentication`, `Modules/Stores`, and `Modules/Billing` own their models, migrations, routes, policies/services, resources, and tests. Billing owns Platform plan/feature administration; it never enters Store context. `app/Support` contains only small cross-cutting adapters (request IDs, store-aware search, media path generation, and dashboard access).
