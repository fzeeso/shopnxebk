...
function middlewareName(string $middleware): string
{
    return str_replace(
        [
            ...
            'Modules\\Tenancy\\Http\\Middleware\\ResolveOptionalTenant',
            'Modules\\Tenancy\\Http\\Middleware\\ResolveTenant',
            'Modules\\Tenancy\\Http\\Middleware\\EnsureTenantMembership',
...
/** @return list<array{type: string, field: string, schema: string, protection: string}> */
...
$purposes = [
    ...
    'spatie/laravel-multitenancy' => 'Tenant lifecycle and tenant-aware jobs.',
    'spatie/laravel-permission' => 'Tenant-team roles and permissions.',
...
foreach (graphQlOperations($root) as $operation) {
...
if ($checkOnly) {
    if (! is_file($output) || file_get_contents($output) !== $rendered) {
        fwrite(STDERR, "Developer-guide inventory is stale. Run `composer docs:update`.\n");
        exit(1);
    }

    fwrite(STDOUT, "Developer-guide inventory is current.\n");
    exit(0);
}
...
fwrite(STDOUT, "Updated docs/generated/system-inventory.md.\n");
